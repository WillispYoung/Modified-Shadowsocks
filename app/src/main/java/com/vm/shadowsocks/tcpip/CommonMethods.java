...
public class CommonMethods {
    public static final String HttpRequestLineFormat = "(?<method>(GET|HEAD|PUT|POST|TRACE|OPTIONS|DELETE)) (?<url>\\S*) HTTP/1.[01]";
...
//            e.printStackTrace();
    ...
    // 检查是否访问恶意域名
    public static boolean checkMaliciousDomain(String host) {
        while (host.length() > 0) { // 利用字典，耗时极少
            if (MalwareInfo.isContainedInSet(host)) {
                System.out.println("malicious domain " + host);
                return true;
            }
            else {
    ...
    // 检查是否分流的URL
    // TODO 应用更确切的分流规则？
    public static boolean checkBypassUrl(String url) {
        return url.matches("/\\S+\\.(mp4|flv|f4v|ts|m3u8|f4vcrc)(\\?\\S+)?");
    }
}
