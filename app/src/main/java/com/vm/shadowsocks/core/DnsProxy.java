...
public class DnsProxy implements Runnable {
    ...
    private int getOrCreateFakeIP(String domainString) {
        ...
        if (fakeIP == null) {
            ...
            do {
                ...
                hashIP++;
            } while (IPDomainMaps.containsKey(fakeIP)); // 逐个尝试，直到伪IP没有出现过

            DomainIPMaps.put(domainString, fakeIP); // 伪IP形如127.25.*.*
            IPDomainMaps.put(fakeIP, domainString);
        }
        return fakeIP;
    }

    //仅在用户发出DNS请求时进行DNS污染操作
    //对于需要代理的域名，创建伪IP作为代替
    ...
    private boolean interceptDns(IPHeader ipHeader, UDPHeader udpHeader, DnsPacket dnsPacket) {
        Question question = dnsPacket.Questions[0];
        System.out.println("DNS Query " + question.Domain);
        if (question.Type == 1) {
